...
public class Shape67 {
    ...
    private TinyPatch lastFound = null; //cache last found to
...
//    }
    ...
    public void generateBilinearMapping() {
        ...
        double v = -stepV;
        float[][] pointColors = new float[4][4];
        for (int i = 0; i < 4; i++) {
            float[] temp = new float[]{colorsArr[i].getRed(), colorsArr[i].getGreen(), colorsArr[i].getBlue(), colorsArr[i].getAlpha()};
            for (int j = 0; j < 4; j++) {
                pointColors[i][j] = (1f / 255f * temp[j]);
            }
        }
        ...
        for (int i = 0; i < szv; i++) {
            ...
            for (int j = 0; j < szu; j++) {
                u += stepU;
                double scx = (1 - v) * C1[j].getX() + v * C2[j].getX();
                double scy = (1 - v) * C1[j].getY() + v * C2[j].getY();

                double sdx = (1 - u) * D1[i].getX() + u * D2[i].getX();
                double sdy = (1 - u) * D1[i].getY() + u * D2[i].getY();

                double sbx = (1 - v) * ((1 - u) * C1[0].getX() + u * C1[C1.length - 1].getX())
                        + v * ((1 - u) * C2[0].getX() + u * C2[C2.length - 1].getX());
                double sby = (1 - v) * ((1 - u) * C1[0].getY() + u * C1[C1.length - 1].getY())
                        + v * ((1 - u) * C2[0].getY() + u * C2[C2.length - 1].getY());

                double sx = scx + sdx - sbx;
                double sy = scy + sdy - sby;

                xy[i][j] = new Point2D.Float((float) sx, (float) sy);

                float[] temp = new float[4];
                for (int ci = 0; ci < 4; ci++) {
                    temp[ci] = (float) ((1 - v) * ((1 - u) * pointColors[0][ci] + u * pointColors[3][ci])
                            + v * ((1 - u) * pointColors[1][ci] + u * pointColors[2][ci]));
                }
    ...
    public Color findPointColor(Point2D p) {
        ...
        if (lastFound != null && lastFound.getPath().contains(p)) {
            ...
            return recurseTrapezoidal(p, points, colors, new ArrayList<Integer>());
        }
    ...
    public Color recurseTrapezoidal(Point2D p, Point2D[] points, Color[] colors, ArrayList<Integer> depth) {
        if (depth.size() > 2) {
            return colors[0];
        }
        ...
        float[][] pointColors = new float[4][4];
        for (int i = 0; i < 4; i++) {
            float[] temp = new float[]{colors[i].getRed(), colors[i].getGreen(), colors[i].getBlue(), colors[i].getAlpha()};
            for (int j = 0; j < 4; j++) {
                pointColors[i][j] = (1f / 255f * temp[j]);
            }
        }
        ...
        ArrayList<TinyPatch> tinyPatches = new ArrayList<TinyPatch>();
        for (int i = 0; i < d; i++) {
            for (int j = 0; j < d; j++) {
                Point2D[] patchPoints = new Point2D[]{xy[i][j], xy[i][j + 1], xy[i + 1][j + 1], xy[i + 1][j]};
                Color[] patchColors = new Color[]{cc[i][j], cc[i][j + 1], cc[i + 1][j + 1], cc[i + 1][j]};
                tinyPatches.add(new TinyPatch(patchPoints, patchColors));
            }
        }

        for (TinyPatch tiny : tinyPatches) {
            if (tiny.getPath().contains(p)) {
                depth.add(1);
                return recurseTrapezoidal(p, tiny.getPoints(), tiny.getColors(), depth);
            }
        }
        return colors[0];
    ...
    private void determineSteps() {
        int len = (int) (Math.max(getC1Length(), getD1Length()));
        nSteps = len < 4 ? 4 : 10;
    }
