...
public class PathSerializer {
	...
	/**
	 * method to serialize a path. This method will iterate through the iterator passed in, and write out the base components of the path to the
	 * ObjectOutput
	 * 
	 * @param os
	 *            - ObjectOutput to write to
	 * @param pi
	 *            - PathIterator path components to iterate over
	 * @throws IOException
	 */
	 ...
	 * 
	 * @param os
	 *            - ObjectInput that contains the serilized path
	...
	public static GeneralPath deserializePath(ObjectInput os) throws ClassNotFoundException, IOException {
		...
		if (windingRule == null) return null;
		...
		/**
		 * iterate over the list, and rebuild the path from the individual movements stored inside the list
		 */
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			int pathType = (Integer) iter.next();
