...
public class TTGlyphs extends PdfJavaGlyphs {
    ...
    public PdfGlyph getTTGlyph(int idx,String glyph,int rawInt, String displayValue, float currentWidth) {
        ...
        try{
            ...
            if(idx!=-1){
                ...
                if(p!=-1){
                        currentGlyph=new TTGlyph(glyph,debug,currentGlyf, fontTable,currentHmtx,idx,(unitsPerEm/1000f),baseFontName);
    ...
    public void setEncodingToUse(boolean hasEncoding, int fontEncoding, boolean isSubstituted, boolean isCIDFont) {
        ...
        if(currentCMAP!=null){
            if(isCorrupted)
                currentCMAP.setEncodingToUse(hasEncoding,fontEncoding,true,isCIDFont);
            else
    ...
    public Map getCharStrings() {
        ...
        if(currentCMAP!=null){
            ...
            return currentGlyf.buildCharStringTable(StandardFonts.MAC);
        }
    }
    ...
    public int readEmbeddedFont(boolean TTstreamisCID, byte[] fontDataAsArray, FontData fontData) {
        ...
        currentLoca=new Loca(currentFontFile,glyphCount,currentHead.getFormat());
