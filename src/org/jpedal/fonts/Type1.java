...
public class Type1 extends PdfFont {
	 ...
	 * System.out.println(glyph+" "+glyphStream.length+" "+charStrings); for(int i=0;i<glyphStream.length;i++) System.out.print((char)
	 * glyphStream[i]); if(glyphStream!=null){ boolean isFirst=true; //flag to pick up extra possible first value int hintCount=0; //String
	 * testChar="xx:";//"x�"; //if((displayValue.equals(testChar))) // test=true; // int p = 0,
	 * nextVal,key=0,lastKey,dicEnd=glyphStream.length,opCount=0,currentOp; double[] op = new double[dicEnd]; //current operand in dictionary as max
	 * size (just in case) float[] pt=new float[6]; if((test)){ System.out.println("******************"+fontID+" "+displayValue); float
	 * h=(float)(FontBBox[3]); current_path.moveTo((float)FontBBox[0],(float)FontBBox[1]); current_path.lineTo((float)FontBBox[0],h);
	 * current_path.lineTo((float)FontBBox[2],h); current_path.lineTo((float)FontBBox[2],(float)FontBBox[1]);
	 * current_path.lineTo((float)FontBBox[0],(float)FontBBox[1]); for(int j=p;j<dicEnd;j++){ int i=glyphStream[j] & 0xff; if(i<charCodes1Byte.length)
	 * System.out.println(" "+i+" "+charCodes1Byte[i]); else System.out.println(" "+i+" "+glyphStream[j]); } System.out.println("==="); }
	 * //current_path.moveTo(0,0); while (p < dicEnd) { nextVal = glyphStream[p] & 0xFF; if (nextVal >31 | (nextVal==28)) { //number,SID or array
	 ...
	 * j=0;j<opCount;j++) System.out.print(" "+op[j]); System.out.print(" "+isFirst+" \n"); } if (key ==12) { //handle escaped keys key=
	 * glyphStream[p] & 0xFF; p++; if(key==34){ //hflex //first curve x += op[0]; pt[0]=(float) x; pt[1]=(float) y; x += op[1]; y += op[2];
	 * pt[2]=(float) x; pt[3]=(float) y; x += op[3]; pt[4]=(float) x; pt[5]=(float) y; current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]);
	 * //second curve x += op[4]; pt[0]=(float) x; pt[1]=(float) y; x += op[5]; pt[2]=(float) x; pt[3]=(float) y; x += op[6]; pt[4]=(float) x;
	 * pt[5]=(float) y; current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]); }else if(key==35){ //flex for(int
	 * curves=0;curves<12;curves=curves+6){ for(int points=0;points<6;points=points+2){ x += op[curves+points]; y += op[curves+points+1];
	 * pt[points]=(float) x; pt[points+1]=(float) y; } current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]); } }else if(key==36){ //hflex1
	 * //first curve x+=op[0]; y+=op[1]; pt[0]=(float) x; pt[1]=(float) y; x+=op[2]; y+=op[3]; pt[2]=(float) x; pt[3]=(float) y; x+=op[4];
	 * pt[4]=(float) x; pt[5]=(float) y; current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]); //second curve x+=op[5]; pt[0]=(float) x;
	 * pt[1]=(float) y; x+=op[6]; y+=op[7]; pt[2]=(float) x; pt[3]=(float) y; x += op[8]; pt[4]=(float) x; pt[5]=(float) y;
	 * current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]); }else if(key==37){ //flex1 double dx = 0, dy = 0,x1=x, y1=y; //workout
	 * dx/dy/horizontal and reset flag for ( int count =0; count <10; count=count+2 ){ dx += op[count]; dy += op[count+1]; } boolean
	 * isHorizontal=(Math.abs(dx)>Math.abs(dy)); for(int points=0;points<6;points=points+2){//first curve x += op[points]; y += op[points+1];
	 ...
	 * boolean isHor = ( key==6 ); int start=0; while (start<opCount ){ if ( isHor ) x += op[start]; else y += op[start];
	 * current_path.lineTo((float)x,(float)y); start++; isHor =!isHor; } }else if(key==8){//rrcurveto int curveCount = ( opCount ) / 6; while (
	 ...
	 * y += op[0]; currentOp++; } //note odd co-ord order while (currentOp<opCount ){ if(isVV) y += op[currentOp]; else x += op[currentOp];
	 * pt[0]=(float) x; pt[1]=(float) y; x += op[currentOp+1]; y += op[currentOp+2]; pt[2]=(float) x; pt[3]=(float) y; if(isVV) y += op[currentOp+3];
	 * else x += op[currentOp+3]; pt[4]=(float) x; pt[5]=(float) y; currentOp += 4; current_path.curveTo(pt[0],pt[1],pt[2],pt[3],pt[4],pt[5]); } }else
	 * if((key==30)|(key==31)){ //vhcurveto/hvcurveto boolean isHor=(key==31); while ( opCount >= 4 ){ opCount -= 4; if ( isHor ) x += op[currentOp];
	 ...
	 * System.out.println(p+">>>>>"+hintCount+">>>>>>"+key+" "+charCodes1Byte[key]+" <1<<"+op); for(int j=0;j<dicEnd;j++)
	 * System.out.println(j+" "+(glyphStream[j] & 0xff)); } } opCount=0; //move to first operator isFirst=false; } } return current_path; }else return
	 * null; }
	 */
	...
	protected final void readType1FontFile(byte[] content) throws Exception {
		...
		if (LogWriter.isOutput()) LogWriter.writeLog("Embedded Type1 font used " + getBaseFontName());
		...
		while (true) {
			...
			if (line.startsWith("/Encoding 256 array")) readDiffEncoding(br);
			else
		...
		int glyphCount = 0;
		if (this.renderPage) glyphCount = readEncodedContent(content);
		this.glyphs.setGlyphCount(glyphCount);

		if (!this.renderPage || glyphCount > 0) this.isFontEmbedded = true;
		...
		while (start < l) {
			...
			if (cont[end] == 47) {
				...
				while (end < total) {
					...
					if (cont[end - 1] == 124 && (cont[end] == 45 || cont[end] == 48) && (cont[end + 1] == 10 || cont[end + 1] == 13)) break;

					if (cont[end - 1] == 'N' && cont[end] == 'D') break;
