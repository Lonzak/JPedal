...
public class ObjectFactory {
	public static PdfObject createObject(int id, String ref, int parentType, int parentID) {
		...
		switch (id) {
			...
			case PdfDictionary.A:
				if (parentType == PdfDictionary.Form) return new FormObject(ref);
				else
					if (parentType == PdfDictionary.MCID) return new MCObject(ref);
					else return new OutlineObject(ref);
			...
			case PdfDictionary.D:
				...
				if (parentType == PdfDictionary.Form) return new FormObject(ref, parentID);
				else return new OCObject(ref);
			...
			case PdfDictionary.JS:
				if (parentType == PdfDictionary.Form) return new FormObject(ref);
				else return new NamesObject(ref);

			case PdfDictionary.K:
				if (parentType == PdfDictionary.MCID) return new MCObject(ref);
				else return new FormObject(ref);
	...
	public static PdfObject createObject(int id, int ref, int gen, int parentType) {
		...
		switch (id) {
			...
			case PdfDictionary.A:
				if (parentType == PdfDictionary.Form) return new FormObject(ref, gen);
				else
					if (parentType == PdfDictionary.MCID) return new MCObject(ref, gen);
					else return new OutlineObject(ref, gen);
			...
			case PdfDictionary.D:
				...
				else return new OCObject(ref, gen);
			...
			case PdfDictionary.JS:
				if (parentType == PdfDictionary.Form) return new FormObject(ref, gen);
				else return new NamesObject(ref, gen);

			case PdfDictionary.K:
				if (parentType == PdfDictionary.MCID) return new MCObject(ref, gen);
				else return new FormObject(ref, gen);
